...
export function toRGB01({h, s, l}: {h: number, s: number, l: number}) {
  s /= 100;
  l /= 100;
...
export function toLightColor(hex: string, shade: string) {
  const { h, s } = toHSL(hex);
  return toRGB01({h, s, l: 100 - Number(shade) / 10});
}

export function toDarkColor(hex: string, shade: string) {
  const { h, s } = toHSL(hex);
